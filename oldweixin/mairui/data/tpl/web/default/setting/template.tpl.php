	...
	<div class="template form-horizontal">
		...
		<li>
			<div class="template-style-pic"> <!--设为默认风格时class中加on-->
				<div class="title"><a onclick="return confirm('确定卸载此模板吗？')" href="<?php  echo create_url('setting/template/uninstall', array('templateid' => $item['name']))?>" class="icon-remove" title="移除"></a><span class="pull-left"><?php  echo $item['title'];?> (<?php  echo $item['name'];?>)</span></div>
		...
		<div class="alert alert-info">未安装的风格</div>
		<ul class="unstyled template-style clearfix">
		...
		<li>
			<div class="template-style-pic">
				...
				<img src="./themes/mobile/<?php  echo $item['name'];?>/preview.jpg" />
				<span class="icon-ok"></span>
				<a href="<?php  echo create_url('setting/template/install', array('templateid' => $item['name']))?>" class="btn btn-danger pull-right" style="position:absolute; bottom:5px; right:5px;">安装</a>
			</div>
		</li>
		<?php  } } ?>
		</ul>
