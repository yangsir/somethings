	...
	<form class="form-horizontal form" action="" method="post" enctype="multipart/form-data" onsubmit="return formcheck()">
			...
			<tr>
				...
				<td>
					<select name="application[type]">
						<?php  if(is_array($modtypes)) { foreach($modtypes as $k => $tp) { ?>
						<option value="<?php  echo $k;?>"<?php  if($k == 'other') { ?> selected="selected"<?php  } ?>><?php  echo $tp;?></option>
						<?php  } } ?>
					</select>
		...
		<table class="tb">
			<tr>
				...
				<td>
					...
					<?php  } } ?>
					<span class="help-block">订阅特定的消息类型后, 此消息类型的消息到达系统后将会以通知的方式(消息数据只读, 并不能返回处理结果)调用模块的接受器, 用这样的方式可以实现全局的数据统计分析等功能.</a></span>
					<div class="alert-warning alert" style="width:auto;margin-top:5px;">注意: 订阅的消息信息是只读的, 只能用作分析统计, 不能更改, 也不能改变处理主流程</div>
				</td>
			</tr>
			<tr>
		...
		<table class="tb">
			...
			<tr>
				...
				<td>
					<ul class="unstyled" id="bindings-<?php  echo $point;?>">
						<li>
							<div class="input-prepend">
								<span class="add-on">操作名称</span>
								<input class="span2" name="bindings[<?php  echo $point;?>][titles][]" type="text" placeholder="请输入操作名称">
							</div>
							<div class="input-prepend">
								<span class="add-on">入口标识</span>
								<input class="span2" name="bindings[<?php  echo $point;?>][dos][]" type="text" placeholder="请输入操作入口">
							</div>
							<div class="input-prepend">
								<span class="add-on">操作附加数据</span>
								<input class="span2" name="bindings[<?php  echo $point;?>][state][]" type="text" placeholder="操作附加数据">
							</div>
							<label class="checkbox inline" style="margin-left:10px;">
					...
					<div class="well well-small" style="margin-top:20px;">
						<a href="javascript:;" onclick="addOption('<?php  echo $point;?>');">添加操作 <i class="icon-plus-sign" title="添加菜单"></i></a>
					</div>
		...
		<table class="tb">
			<tr>
				<th><label for="">兼容的版本</label></th>
				<td>
					<?php  if(is_array($versions)) { foreach($versions as $v) { ?>
					<label class="checkbox">
						<input name="versions[]" type="checkbox" value="<?php  echo $v;?>" <?php  if(in_array($v, $m['versions'])) { ?> checked="checked"<?php  } ?> /> <?php  echo $v;?>
					</label>
					<?php  } } ?>
