...
function paginationM($tcount, $pindex, $psize = 15, $url = '', $context = array('before' => 5, 'after' => 4, 'ajaxcallback' => '')) {
	...
	$html = '<div class="mobilepager"><table><tr>';
	if(true) {
		$html .= "<td><a {$pdata['faa']} class=\"pager-nav\">首页</a></td>";
		$html .= "<td><a {$pdata['paa']} class=\"pager-nav\">&laquo;上一页</a></td><td><a><font color='red'>$cindex</font>/{$pdata['tpage']}</a></td>";
	}
	//页码算法：前5后4，不足10位补齐
	...
	if(true) {
		$html .= "<td><a {$pdata['naa']} class=\"pager-nav\">下一页&raquo;</a></td>";
		$html .= "<td><a {$pdata['laa']} class=\"pager-nav\">尾页</a></td>";
	}
	$html .= '</tr></table></div>';
	return $html;
}
