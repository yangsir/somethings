...
function account_module($enabledOnly = true) {
	...
	if (!empty($founder) && in_array($uid, $founder)) {
		...
		$membermodules = pdo_fetchall("SELECT b.* FROM ".tablename('modules')." AS b LEFT JOIN ".tablename('members_permission')." AS a ON a.resourceid = b.mid WHERE a.uid = :uid AND b.name <> '' OR b.issystem = 1 ".(!empty($groupsmodules['modules']) ? ' OR b.mid IN ('.implode(',', $groupsmodules['modules']).')' : '')." ORDER BY issystem DESC, mid ASC", array(':uid' => $uid), 'mid');
	}
...
function account_weixin_basic() {
	...
	!empty($image['content']) && file_write('headimg_'.$wechat['weid'].'.jpg', $image['content']);
	$image = account_weixin_http($wechat['username'], WEIXIN_ROOT . '/cgi-bin/getqrcode?fakeid='.$fakeid.'&style=1&action=download');
	!empty($image['content']) && file_write('qrcode_'.$wechat['weid'].'.jpg', $image['content']);
	...
	if (strexists($response['content'], '服务号')) {
