...
class WeEngine {
	...
	public function start() {
		...
		if(!WeUtility::checkSign($this->token)) {
			//此处为临时解决微信公众平台出现短暂的延迟或者是不回复的问题。
			//此办法可能会带来接口安全的问题，如果需要还原则去掉下条语句的注释即可（注释即exit前面的两条//）。
			//exit('Access Denied');
		}
		...
		if(strtolower($_SERVER['REQUEST_METHOD']) == 'post') {
			...
			WeSession::start();			 $_SESSION['nower']=$this->message['from'];
	...
	private function matcherText($order = -1) {
		$pars = array();
		...
		global $_W;
		$order = intval($order);
		$condition = "`status`=1 AND (`weid`='{$_W['weid']}' OR `weid`=0) AND `displayorder`>{$order}";
...
abstract class WeModuleSite {
	...
	protected function payResultQuery($tid) {
		$sql = 'SELECT * FROM ' . tablename('paylog') . ' WHERE `module`=:module AND `plid`=:plid';
		$params = array();
		$params[':module'] = $this->module['name'];
		$params[':plid'] = $plid;
