...
class FansModuleSite extends WeModuleSite {
	...
	public function doWebDisplay() {
		...
		$list = pdo_fetchall("SELECT * ".(!empty($select) ? ",`".implode('`,`', $select)."`" : '')." FROM ".tablename('fans')." WHERE follow = 1 AND weid = '{$_W['weid']}' AND from_user <> '' $where ORDER BY id DESC LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
