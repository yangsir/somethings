...
class UserapiModuleProcessor extends WeModuleProcessor {
	...
	private function procRemote($item) {
		...
		if($this->message['type'] == 'text') {
			...
			$body = "<xml><ToUserName><![CDATA[{$this->message['to']}]]></ToUserName><FromUserName><![CDATA[{$this->message['from']}]]></FromUserName><CreateTime>{$this->message['time']}</CreateTime><MsgType><![CDATA[text]]></MsgType><Content><![CDATA[{$this->message['content']}]]></Content><MsgId>{$this->message['msgid']}</MsgId></xml>";
		}
